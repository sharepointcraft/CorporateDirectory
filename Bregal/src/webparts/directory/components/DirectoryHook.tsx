...
const DirectoryHook: React.FC<IDirectoryProps> = (props) => {
    ...
    state.users && state.users.length > 0
        ? state.users.map((user: any) => {
            ...
            // }
            if (user.CompanyName && departments.indexOf(user.CompanyName) < 0) {
                if(user.CompanyName.indexOf("Good Energies") < 0)
                {
                    departments.push(user.CompanyName);
                } 
            }
    ...
    const _searchByAlphabets = async (initialSearch: boolean, locationparam?: string, departmentparam?: string, jobtitleparam?: string) => {
        ...
        if (initialSearch || (`${alphaKey}` == "All" && (!locationKey && !locationparam) && (!jobTitleKey && !jobtitleparam) && (!departmentKey && !departmentparam))) {
            ...
            let refinerQueryString = `AND (OfficeNumber:${location} OR BaseOfficeLocation:${location}) ,CompanyName:${department}`;
            ...
            .then((client: AadHttpClient) => {
                ...
                return client
                    .get(
                        ...
                        `https://graph.microsoft.com/v1.0/users?$select=id,displayname,mail,officeLocation,companyName,onPremisesDistinguishedName&$top=999`,
    ...
    const _apUserGraphArray = (json: any): void => {
        ...
        Promise.all(
            json.value.map(x => {
                if (x.onPremisesDistinguishedName != null) {
                    if (x.onPremisesDistinguishedName.indexOf("OU=Leavers") <= 0) {
                        if (x.onPremisesDistinguishedName.indexOf('OU=Users') >= 0) {
                            if (x.onPremisesDistinguishedName.toLowerCase().indexOf("conference") <= 0) {
                                if (x.onPremisesDistinguishedName.toLowerCase().indexOf("admin accounts") <= 0) {
                                    if (x.onPremisesDistinguishedName.toLowerCase().indexOf("adm_") <= 0) {
                                        if (x.onPremisesDistinguishedName.toLowerCase().indexOf("test") <= 0) {
                                            if (x.onPremisesDistinguishedName.toLowerCase().indexOf("freshstream") <= 0) {
                                                if (x.onPremisesDistinguishedName.toLowerCase().indexOf("emkcapital") <= 0) {
                                                    userArray.push({
            ...
            //Filter for Users
            var removeUserArray = ["Adam Frahm", "Quentin Van Doosselaere", "Steven Black", "Tarquin Wethered", "Toni Martello", "Julia Maurice", "Marina Knight", "Tanya Clark", "Nicole Wangemann", "Ejike Onuchukwu", "Patrik Johnson", "General@bregal.it", "Alexander Acosta", "Lindsey Ascheman", "Steve Mahedy", "Casey Kerr", "Gregory Peter"];
    ...
    return (
        <div className={styles.directory}>
            ...
            <div className={styles.searchBox} style={{ paddingLeft: '20%' }}>
                {/* <select id="searchPeopleBy" className={styles.searchPeopleByddl}>
                    <option value="FirstName">By Name</option>
                    <option value="CompanyName">By Company</option>
                </select> */}
                                        ...
                                        <div style={{ width: '20%', display: 'inline-block' }}>
                                            <Stack horizontal horizontalAlign="center" wrap tokens={wrapStackTokens}>
                                                <div style={{ width: '65%' }}>
                                                    ...
                                                    <h4 style={{ marginBottom: '6px' }}>Company</h4>
                                                    <div className={styles.refinerSection}>
                                                        {refinerDepartmentGrid}
                                                    </div>
